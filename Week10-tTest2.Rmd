...
1.  Conduct independent samples (two-sample) t-tests
2.  Generate figures
...
Remember that any line with a \# makes a comment and the code does not run. Below is how to load the .csv data from a local file (you need to change the file path to where the local file is). The data contains all of the measures and conditions from Experiment 4.
...
a.  Explain the difference between a paired-samples t-test and an independent samples t-test. ( 2 points )

b.  Imagine you were to conduct a between-subjects experiment with two groups, where the manipulation had no effect whatsoever. If you repeated this experiment (that doesn't work) 100 times and computed a t-test each time, approximately how many of the experiments out of 100 would you expect find the associated p-value is .05 or smaller? Explain (2 points)

### Stretch Problems (required this time - 4 points)
...
General grading for written portion.
